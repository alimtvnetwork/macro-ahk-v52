...
**Scope:** `spec/01-prompt-spec-2026/`
...
|---|---|---|---|
| 1 | New canonical root exists with `NN-` prefix | `ls -d spec/01-prompt-spec-2026/` | ✅ present |
| 2 | Root matches repo-wide convention | `ls spec/ \| grep -cE '^[0-9]{2}-'` → 34 NN-prefixed siblings | ✅ 01-prompt-spec-2026 fits |
| 3 | Old root removed from active tree | `find spec/2026-spec -type f` → 1 file (redirect stub only) | ✅ no live content |
| 4 | Redirect stub points to new root | `spec/2026-spec/README.md` opens with “Moved — see `spec/01-prompt-spec-2026/`” | ✅ external bookmarks caught |
| 5 | No tooling references old root as authoritative | `grep -rn "spec/2026-spec/" spec/01-prompt-spec-2026/` → only 1 historical mention in `300-blind-ai-rescore-pre-renumber.md` (audit record) | ✅ no live dependency |
| 6 | `scripts/spec/apply-rename-map.mjs` documents the rename pair | `ROOT_PAIR = ['spec/2026-spec/','spec/01-prompt-spec-2026/']` | ✅ auditable |
...
| # | Verification | Evidence | Outcome |
|---|---|---|---|
| 9 | Inner folders renumbered to dense `01..20` | `ls -d spec/01-prompt-spec-2026/*/` → `01-glossary, 02-data-model, …, 20-adoption-checklist, 99-spec-issues` | ✅ dense |
...
| 14 | All 107 files preserved across rename | `find spec/01-prompt-spec-2026 -type f \| wc -l` = 107; pre-rename inventory `inventory-before.txt` = 105 + 2 new audit artifacts | ✅ no loss |
...
| 22 | No dangling old-root paths in active spec | `grep -rn "2026-spec/" spec/01-prompt-spec-2026/` returns only 1 historical line + 1 poc/ reference in hardening backlog (out-of-scope code path) | ✅ |
...
| 25 | Spec-organization memory upgraded to multi-tree index | `mem://architecture/spec-organization.md` adds `spec/01-prompt-spec-2026/` as a top-level root with dense `NN-name` rule (Phase F step 83) | ✅ |
...
ls -d spec/01-prompt-spec-2026/*/ | sort
ls spec/ | grep -E '^01-prompt-spec-2026$'
```
...
- ✅ Root folder now `spec/01-prompt-spec-2026/` (NN-prefixed, consistent with 34 sibling roots)
