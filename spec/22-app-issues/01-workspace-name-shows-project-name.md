...
The MacroLoop controller's status bar (yellow highlighted text) intermittently displayed the **project name** (e.g., "macro-ahk-v50") instead of the **workspace name** (e.g., "P07 D2V7 Orinmax's Lovable v5").
...
- The yellow-highlighted name in the status bar showed "macro-ahk-v50" (the project name) instead of the actual workspace name.
...
User reported that the highlighted section sometimes shows the project name instead of the workspace name. Screenshot showed "macro-ahk-v50" highlighted in yellow in the MacroLoop controller status bar.
...
The project name "macro-ahk-v50" does not appear in the workspace list (workspaces have names like "P07 D2V7 Orinmax's Lovable v5"). By validating against the list, the observer will reject "macro-ahk-v50" and leave `state.workspaceName` empty until the API correctly identifies the workspace.
