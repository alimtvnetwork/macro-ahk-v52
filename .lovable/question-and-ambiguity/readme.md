...
- [58 — 2026 Spec — scope & placeholders](./58-2026-spec-scope-placeholders.md) — Created `spec/2026-spec/01-prompt-spec/` (now `spec/2026-spec/01-prompt-spec/`) with 00-overview + 01-plan-tasks-1-20; all host XPaths left as `???`; cadence: 10 spec tasks per `next ten steps` turn (10 turns to cover T21–T120).
- [59 — 2026-spec folder name finalization](./59-2026-spec-folder-name.md) — Finalized the requested folder name: completed spec NOW lives at `spec/2026-spec/01-prompt-spec/` (renumbered 2026-06-03; PoC dir was never created — N/A), and scripts/docs point there.
- [60 — PERF batch: write RCAs only vs. land code fixes](./60-perf-batch-rca-vs-fix.md) — Chose Option 1: wrote 7 RCAs under `spec/22-app-issues/10..16-…md` per the plan's "record findings only" gate; awaiting greenlight before code edits.
