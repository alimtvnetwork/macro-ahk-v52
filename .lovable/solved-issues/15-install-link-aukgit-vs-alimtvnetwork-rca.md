...
→ 404 / asset not found. v50 equivalent (`alimtvnetwork/macro-ahk-v50`) worked.

## Root cause

The canonical published repo for v51 is **`alimtvnetwork/macro-ahk-v51`** (same
owner pattern as v50 — `alimtvnetwork/macro-ahk-v50`). An earlier change
inverted the assumption and treated `aukgit/macro-ahk-v51` as canonical:

- `readme.md` install/clone/install-from-release URLs pointed to `aukgit/...`.
- `scripts/download-extension.ps1` defaulted `-Repo` to `aukgit/macro-ahk-v51`.
- `scripts/clone-ahk.mjs` and `scripts/clone-repo.ps1` actively **rewrote**
  `alimtvnetwork/macro-ahk-v51` → `aukgit/macro-ahk-v51` (backwards).

Releases are published under `alimtvnetwork/macro-ahk-v51`, so every
...
`difflib` against
`https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v50/main/scripts/download-extension.ps1`
...
1. `scripts/download-extension.ps1` — `-Repo` default reverted to
   `alimtvnetwork/macro-ahk-v51`; docstring updated.
2. `readme.md` — every `aukgit/macro-ahk-v51` occurrence (install commands,
   clone URLs, badge URLs, release URLs) rewritten to
   `alimtvnetwork/macro-ahk-v51`.
...
Add to memory: **`alimtvnetwork/macro-ahk-v5x` is the canonical published
