...
It runs locally and is correctly wired into `package.json` as `check:installer-contract`, but it was **not** invoked by the CI workflow. A future drift between the two installers (like the long-standing default-repo bug — `install.ps1` hardcoded to `macro-ahk-v36` while `install.sh` used `macro-ahk-v36`) could land on `main` without being caught.
