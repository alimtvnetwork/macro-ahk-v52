...
|---|------|--------|--------|---------|
| 1 | **Priority 0.8** — staged authored-source id-denylist quarantine retirement (`cb`, `obj`, `fn`, `el`, `msg`, `ctx`) | High | Medium — generated-output noise fixed; placeholder identifiers now enforced for new/cleaned files with complete legacy quarantine | Effort budget |
...
**Recommended next**: Priority 0.8 quarantine retirement — remove legacy `cb`/`obj`/`fn`/`el`/`msg`/`ctx` entries file-by-file after descriptive identifier renames. Defer Priority 5 until checksum strategy is approved.
