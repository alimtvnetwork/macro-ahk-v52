...
```powershell
$env:MARCO_DL_VERSION='v3.91.0'; $env:MARCO_DL_FOLDER='marco'; irm https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v50/main/scripts/download-extension.ps1 | iex
```

**Windows · PowerShell — run a local clone with explicit flags:**

```powershell
.\scripts\download-extension.ps1 -Version v3.91.0 -FolderName marco-extension
```

After it finishes, load the resulting folder via `chrome://extensions → Load unpacked`. The `marco-extension-v3.91.0.zip` backup sits next to it.
...
### 📌 Pinned version (`v3.91.0`)

```powershell
# Windows · PowerShell
irm https://github.com/alimtvnetwork/macro-ahk-v50/releases/download/v3.91.0/install.ps1 | iex
...
```bash
# macOS · Linux · Bash
curl -fsSL https://github.com/alimtvnetwork/macro-ahk-v50/releases/download/v3.91.0/install.sh | bash


```

**Pinned version:** `v3.91.0` &nbsp;•&nbsp; **Macro Controller:** `v3.91.0`
...
- 🔒 Pinned to `v3.91.0` — see [Pinned to a specific release](#-pinned-to-a-specific-release) to change versions
...
- 🔒 Pinned to `v3.91.0` — see [Pinned to a specific release](#-pinned-to-a-specific-release) for other tags
...
```powershell
# PowerShell (Windows) — replace v3.91.0 with any released tag
irm https://github.com/alimtvnetwork/macro-ahk-v50/releases/download/v3.91.0/install.ps1 | iex
```
...
|------|-------------|---------|
| `-Version` | Pin a specific release (`vX.Y.Z[-pre]`) or `latest` | `-Version v3.91.0` |
...
|------|-------|-------------|---------|
| `--version <ver>` | `-v` | Force a specific version (`vX.Y.Z[-pre]`) or `latest`. Anything else exits 3. | `--version v3.91.0` |
| `--dir <path>` | `-d` | Target directory (default: `~/marco-extension`) | `--dir ~/marco-extension/v3.91.0` |
| `--repo <o/r>` | `-r` | Override GitHub `owner/repo` | `--repo alimtvnetwork/macro-ahk-v50` |
| `--dry-run` | — | Resolve the install plan, print it, exit 0 — **nothing is downloaded or extracted**. Useful for CI smoke tests and debugging. | `--dry-run --version v3.91.0` |
...
./install.sh --version v3.91.0 --dir ~/marco-extension/v3.91.0
...
2. Extract to a folder (e.g., `D:\marco-extension\v3.91.0`)
...
Creating or pushing a `release/*` branch (e.g., `release/v3.91.0`) or creating a
