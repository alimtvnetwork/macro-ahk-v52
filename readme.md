...
&#x200B;<!-- Build & Release --> [![CI](https://img.shields.io/github/actions/workflow/status/alimtvnetwork/macro-ahk-v52/ci.yml?branch=main&label=CI&logo=github&style=flat-square)](https://github.com/alimtvnetwork/macro-ahk-v52/actions/workflows/ci.yml) <!-- Repo activity --> [![Issues](https://img.shields.io/github/issues/alimtvnetwork/macro-ahk-v52?style=flat-square&logo=github)](https://github.com/alimtvnetwork/macro-ahk-v52/issues) [![Pull Requests](https://img.shields.io/github/issues-pr/alimtvnetwork/macro-ahk-v52?style=flat-square&logo=github)](https://github.com/alimtvnetwork/macro-ahk-v52/pulls) [![Repo Size](https://img.shields.io/github/repo-size/alimtvnetwork/macro-ahk-v52?style=flat-square)](https://github.com/alimtvnetwork/macro-ahk-v52)
&#x200B;<!-- Community --> <!-- (intentionally empty — see mem://constraints/no-static-mockup-badges) --> <!-- Code-quality --> [![Security Issues](https://img.shields.io/github/issues-search/alimtvnetwork/macro-ahk-v52?query=is%3Aissue%20label%3Asecurity&label=security&style=flat-square&logo=github)](https://github.com/alimtvnetwork/macro-ahk-v52/issues?q=is%3Aissue+label%3Asecurity) [![Dependency PRs](https://img.shields.io/github/issues-pr-raw/alimtvnetwork/macro-ahk-v52?label=dependency%20PRs&style=flat-square&logo=dependabot)](https://github.com/alimtvnetwork/macro-ahk-v52/pulls?q=is%3Apr+label%3Adependencies) <!-- Stack & standards --> [![License](https://img.shields.io/github/license/alimtvnetwork/macro-ahk-v52?style=flat-square)](#license)
...
```powershell
irm https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v52/main/scripts/download-extension.ps1 | iex
```

**Windows · PowerShell — pin a version + custom folder name (env-var form, works with `irm | iex`):**

```powershell
$env:MARCO_DL_VERSION='v4.11.0'; $env:MARCO_DL_FOLDER='marco'; irm https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v52/main/scripts/download-extension.ps1 | iex
```

**Windows · PowerShell — run a local clone with explicit flags:**

```powershell
.\scripts\download-extension.ps1 -Version v4.11.0 -FolderName marco-extension
```

**Windows · PowerShell — source checkout without full-history clone:**

```powershell
irm https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v52/main/scripts/clone-repo.ps1 | iex
```

After it finishes, load the resulting folder via `chrome://extensions → Load unpacked`. The `marco-extension-v4.11.0.zip` backup sits next to it.

### 🪟 Windows · PowerShell installer (latest)

```powershell
irm https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v52/main/scripts/install.ps1 | iex
```

### 🐧 macOS · Linux · Bash installer (latest)

```bash
curl -fsSL https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v52/main/scripts/install.sh | bash
```

### 📌 Pinned version (`v4.11.0`)

```powershell
# Windows · PowerShell
irm https://github.com/alimtvnetwork/macro-ahk-v52/releases/download/v4.11.0/install.ps1 | iex
...
```bash
# macOS · Linux · Bash
curl -fsSL https://github.com/alimtvnetwork/macro-ahk-v52/releases/download/v4.11.0/install.sh | bash


```

**Pinned version:** `v4.11.0` &nbsp;•&nbsp; **Macro Controller:** `v4.11.0`
...
```powershell
irm https://github.com/alimtvnetwork/macro-ahk-v52/releases/download/v4.9.1/install.ps1 | iex


```

- 🔒 Pinned to `v4.9.1` — see [Pinned to a specific release](#-pinned-to-a-specific-release) to change versions
...
```bash
curl -fsSL https://github.com/alimtvnetwork/macro-ahk-v52/releases/download/v4.9.1/install.sh | bash


```

- 🔒 Pinned to `v4.9.1` — see [Pinned to a specific release](#-pinned-to-a-specific-release) for other tags
...
```powershell
# PowerShell (Windows) — replace v4.9.1 with any released tag
irm https://github.com/alimtvnetwork/macro-ahk-v52/releases/download/v4.9.1/install.ps1 | iex
```
...
& ([scriptblock]::Create((irm https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v52/main/scripts/install.ps1))) -InstallDir (Join-Path $PWD 'marco-extension')
```

```bash
# Bash (Linux / macOS) — installs to ./marco-extension under the current folder
curl -fsSL https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v52/main/scripts/install.sh | bash -s -- --dir "$PWD/marco-extension"
```
...
```powershell
& { $Version = "v2.220.0"; irm https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v52/main/scripts/install.ps1 | iex }
```

```bash
curl -fsSL https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v52/main/scripts/install.sh | bash -s -- --version v2.220.0
```
...
|------|-------------|---------|
| `-Version` | Pin a specific release (`vX.Y.Z[-pre]`) or `latest` | `-Version v4.9.1` |
| `-InstallDir` | Custom install directory | `-InstallDir D:\marco-extension\v3.6.1` |
| `-Repo` | Override GitHub `owner/repo` | `-Repo alimtvnetwork/macro-ahk-v52` |
...
|------|-------|-------------|---------|
| `--version <ver>` | `-v` | Force a specific version (`vX.Y.Z[-pre]`) or `latest`. Anything else exits 3. | `--version v4.9.1` |
| `--dir <path>` | `-d` | Target directory (default: `~/marco-extension`) | `--dir ~/marco-extension/v4.9.1` |
| `--repo <o/r>` | `-r` | Override GitHub `owner/repo` | `--repo alimtvnetwork/macro-ahk-v52` |
| `--dry-run` | — | Resolve the install plan, print it, exit 0 — **nothing is downloaded or extracted**. Useful for CI smoke tests and debugging. | `--dry-run --version v4.9.1` |
...
When the project ships successive major rewrites in sibling repos (e.g. `macro-ahk-v52` → `macro-ahk-v52` → `macro-ahk-v52`), discovery mode can optionally probe ahead and pick the newest. Defaults live in [`scripts/install.config.sh`](scripts/install.config.sh) and are **off by default**:
...
curl -fsSL https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v52/main/scripts/install.sh \
  | bash -s -- --dry-run

# Pin a specific version into a custom directory:
./install.sh --version v4.9.1 --dir ~/marco-extension/v4.9.1
...
1. Download `marco-extension-v{VERSION}.zip` from [Releases](https://github.com/alimtvnetwork/macro-ahk-v52/releases)

2. Extract to a folder (e.g., `D:\marco-extension\v4.9.1`)
...
```bash
git clone --depth=1 --single-branch --filter=blob:none --no-tags https://github.com/alimtvnetwork/macro-ahk-v52.git "macro-ahk"
```
...
This creates a `macro-ahk/` folder containing the AHK v2 scripts that pair with the Chrome extension's macro controller. The package script also rewrites the stale `alimtvnetwork/macro-ahk-v52` owner to `alimtvnetwork/macro-ahk-v52` before cloning.
...
Creating or pushing a `release/*` branch (e.g., `release/v4.9.1`) or creating a
...
**No email or notification is sent** — check the [Releases page](https://github.com/alimtvnetwork/macro-ahk-v52/releases) for status.
...
```powershell
irm https://github.com/alimtvnetwork/macro-ahk-v52/releases/download/v{VER}/install.ps1 | iex
```

**Linux / macOS**

```bash
curl -fsSL https://github.com/alimtvnetwork/macro-ahk-v52/releases/download/v{VER}/install.sh | bash
```
