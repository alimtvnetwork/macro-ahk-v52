...
```powershell
$env:MARCO_DL_VERSION='v3.47.0'; $env:MARCO_DL_FOLDER='marco'; irm https://raw.githubusercontent.com/alimtvnetwork/macro-ahk-v44/main/scripts/download-extension.ps1 | iex
```
...
```powershell
# Windows · PowerShell
irm https://github.com/alimtvnetwork/macro-ahk-v44/releases/download/v3.47.0/install.ps1 | iex
...
```bash
# macOS · Linux · Bash
curl -fsSL https://github.com/alimtvnetwork/macro-ahk-v44/releases/download/v3.47.0/install.sh | bash


```

**Pinned version:** `v3.47.0` &nbsp;•&nbsp; **Macro Controller:** `v3.47.0`
...
|------|-------------|---------|
| `-Version` | Pin a specific release (`vX.Y.Z[-pre]`) or `latest` | `-Version v3.47.0` |
...
|------|-------|-------------|---------|
| `--version <ver>` | `-v` | Force a specific version (`vX.Y.Z[-pre]`) or `latest`. Anything else exits 3. | `--version v3.47.0` |
...
| `--dry-run` | — | Resolve the install plan, print it, exit 0 — **nothing is downloaded or extracted**. Useful for CI smoke tests and debugging. | `--dry-run --version v3.47.0` |
...
./install.sh --version v3.47.0 --dir ~/marco-extension/v3.47.0
...
2. Extract to a folder (e.g., `D:\marco-extension\v3.47.0`)
