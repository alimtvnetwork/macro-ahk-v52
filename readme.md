...
- 🔒 Pinned to `v4.11.0` — see [Pinned to a specific release](#-pinned-to-a-specific-release) to change versions
...
- 🔒 Pinned to `v4.11.0` — see [Pinned to a specific release](#-pinned-to-a-specific-release) for other tags
...
```powershell
# PowerShell (Windows) — replace v4.11.0 with any released tag
irm https://github.com/alimtvnetwork/macro-ahk-v52/releases/download/v4.11.0/install.ps1 | iex
```
...
|------|-------------|---------|
| `-Version` | Pin a specific release (`vX.Y.Z[-pre]`) or `latest` | `-Version v4.11.0` |
...
|------|-------|-------------|---------|
| `--version <ver>` | `-v` | Force a specific version (`vX.Y.Z[-pre]`) or `latest`. Anything else exits 3. | `--version v4.11.0` |
| `--dir <path>` | `-d` | Target directory (default: `~/marco-extension`) | `--dir ~/marco-extension/v4.11.0` |
| `--repo <o/r>` | `-r` | Override GitHub `owner/repo` | `--repo alimtvnetwork/macro-ahk-v52` |
| `--dry-run` | — | Resolve the install plan, print it, exit 0 — **nothing is downloaded or extracted**. Useful for CI smoke tests and debugging. | `--dry-run --version v4.11.0` |
...
./install.sh --version v4.11.0 --dir ~/marco-extension/v4.11.0
...
2. Extract to a folder (e.g., `D:\marco-extension\v4.11.0`)
...
Creating or pushing a `release/*` branch (e.g., `release/v4.11.0`) or creating a
